...
class MetaNetFull(chainer.Chain):
	...
	def embed_key(self, train, support_sets, support_lbls, x_set):
		...
		for i in range(0, N, int(N/IT)):
			...
			x = mod.concatenate(support_sets[i:(i+int(N/IT))], axis=0)
			x = Variable(x)
			x = F.reshape(x, (-1, 1, 28, 28))
			x = F.dropout(x, ratio=0.0)
			...
			y_batch = mod.array(support_lbls[i:int(i+N/IT)], dtype=np.int32).reshape((-1,))
			...
			meta_outs = model.meta_lstm_l1(F.dropout(meta_in, ratio=0.0))
			meta_outs = model.meta_lstm_ll1(F.dropout(meta_outs, ratio=0.0))
			...
			meta_outs1 = model.meta_g_lstm_l1(F.dropout(meta_in, ratio=0.0))
			meta_outs1 = model.meta_g_lstm_ll1(F.dropout(meta_outs1, ratio=0.0))
	...
	def __forward(self, train, support_sets, support_lbls, x_set, x_lbl = None):
		...
		for i in range(N):
			...
			meta_outs = F.relu(model.m_l1(F.dropout(meta_in, ratio=0.0)))
			meta_outs = F.relu(model.m_ll1(F.dropout(meta_outs, ratio=0.0)))
			meta_outs = model.meta_lstm_l2(F.dropout(meta_outs, ratio=0.0))
			...
			meta_outs = F.relu(model.mc_l1(F.dropout(meta_in, ratio=0.0)))
			meta_outs = F.relu(model.mc_ll1(F.dropout(meta_outs, ratio=0.0)))
			meta_outs = model.meta_g_lstm_l2(F.dropout(meta_outs, ratio=0.0))
